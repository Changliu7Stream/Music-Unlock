...
def _env_hex(name: str) -> Optional[bytes]:
    ...
    try:
        ...
        print(f"[decrypt_algorithms] 警告: 环境变量 {name} 的 hex 值无效: {e}")
        return None
...
def _env_int(name: str, default: int) -> int:
    ...
    try:
        ...
        print(f"[decrypt_algorithms] 警告: 环境变量 {name} 不是有效整数: {val!r}，使用默认值 {default}")
        return default
...
def detect_audio_format(data: bytes, fallback: str = "mp3") -> str:
    """
    通过解密后数据的 magic bytes 检测实际音频格式。
    优先使用内容检测，避免依赖文件名猜测。
    """
...
def _decrypt_ncm(file_data: bytes, filename: str) -> tuple[bytes, str, dict]:
    ...
    try:
        ...
        if tag:
            if tag.format:
                audio_format = str(tag.format).lower()
            if tag.musicName:
...
def _decrypt_kgm(file_data: bytes, filename: str) -> tuple[bytes, str, dict]:
    ...
    # 通过 magic bytes 检测实际音频格式，避免硬编码 mp3
    audio_format = detect_audio_format(audio_data)
    return audio_data, audio_format, {}
