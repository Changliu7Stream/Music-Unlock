"""
decrypt_algorithms.py — 音乐解锁后端解密算法模块

从 Music-Unlock 仓库迁移至本独立后端仓库，包含：
  - 密钥加载（环境变量注入，全部带 try-except 错误处理）
  - detect_audio_format() 通过 magic bytes 检测解密后的真实音频格式
  - 各格式解密器（NCM / QMCv1 / QMCv2 / KGMorVPR / KWM）
  - FORMAT_REGISTRY 格式注册表（新后端 ↔ 旧前端解密器映射）
  - route_and_decrypt 路由函数

关联前端项目：
  前端仓库:        https://github.com/Changliu7Stream/Music-Unlock
  前端补丁:        public/api-decrypt.js (拦截旧 Worker 解密，改调后端 /api/decrypt)

关联旧前端解密器说明（um-web legacy v1.10.7 src/decrypt/）：
...
新算法基于 libtakiyasha 2.1.1.post1，密钥全部通过环境变量注入，不硬编码私钥。
KGM/KWM/QMCv1/QMCv2 解密后统一使用 detect_audio_format() 检测真实格式（不硬编码 mp3）；
NCM 使用 ncm_tag.format 并经 str() 类型安全转换，缺失时回退到 magic bytes 检测。
"""
import io
import os
...
def _env_hex(name: str) -> Optional[bytes]:
    """从环境变量读取 hex 编码的密钥，返回 bytes。

    未设置时返回 None；解析失败时打印警告并返回 None（不抛异常，保证服务可启动）。
    """
    ...
    try:
        ...
        print(f"[decrypt_algorithms] 警告: 环境变量 {name} 的 hex 值解析失败: {e}")
        return None


def _env_str(name: str) -> Optional[bytes]:
    """从环境变量读取字符串密钥，返回 UTF-8 编码的 bytes。

    未设置时返回 None；编码失败时打印警告并返回 None。
    """
    val = os.environ.get(name)
    if not val:
        return None
    try:
        ...
        print(f"[decrypt_algorithms] 警告: 环境变量 {name} 的值编码失败: {e}")
        return None
...
def _env_int(name: str, default: int) -> int:
    ...
    try:
        ...
        print(f"[decrypt_algorithms] 警告: 环境变量 {name} 不是合法整数 ({e})，使用默认值 {default}")
        return default
...
# QMCv2 core_key：优先使用直接提供的 hex 值，否则通过 salt+length 派生
QMCV2_CORE_KEY: Optional[bytes] = None
if os.environ.get("QMCV2_CORE_KEY"):
    ...
    try:
        ...
        print(f"[decrypt_algorithms] 警告: QMCv2 core_key 派生失败 ({e})，请检查 QMCV2_CORE_KEY_SALT / QMCV2_CORE_KEY_LENGTH")
...
# ---------------------------------------------------------------------------
# 音频格式检测：通过 magic bytes 识别解密后的真实格式
# ---------------------------------------------------------------------------

def detect_audio_format(data: bytes) -> str:
    """通过 magic bytes 检测解密后的音频格式。

    返回标准化的格式名：flac / ogg / mp3 / m4a / wav / ape。
    无法识别时返回 "mp3" 作为兜底（最常见的格式）。

    各格式 magic bytes：
      FLAC  -> "fLaC"            (offset 0)
      OGG   -> "OggS"            (offset 0)
      WAV   -> "RIFF"...."WAVE"  (offset 0 + offset 8)
      M4A   -> "ftyp"            (offset 4，MP4 容器)
      APE   -> "MAC "            (offset 0)
      MP3   -> "ID3"             (ID3v2 标签，offset 0)
            -> 0xFF 0xE?/0xF?    (MPEG 帧同步，第二字节高 3 位为 1)
    """
    if not data:
        return "mp3"
    ...
    if head[:4] == b"RIFF" and head[8:12] == b"WAVE":
        return "wav"
    # MP4 / M4A（ISO BMFF：4 字节 size + "ftyp"）
    if head[4:8] == b"ftyp":
        return "m4a"
    # APE (Monkey's Audio)
    if head[:4] == b"MAC ":
    ...
    if head[:3] == b"ID3":
        return "mp3"
    # MP3 — MPEG 音频帧同步（11 位全 1：0xFF 且第二字节高 3 位为 1）
...
def _decrypt_ncm(file_data: bytes, filename: str) -> tuple[bytes, str, dict]:
    ...
    try:
        tag = ncmfile.ncm_tag
        if tag:
            # tag.format 使用 str() 类型安全转换（兼容 bytes / 其他类型）
            if tag.format:
                audio_format = str(tag.format)
            if tag.musicName:
                metadata["title"] = str(tag.musicName)
            if tag.artist:
                artists = []
                for a in tag.artist:
            ...
            if tag.album:
                metadata["album"] = str(tag.album)
    except Exception as e:
        print(f"[decrypt_algorithms] NCM 元数据读取失败: {e}")

    # 未从 tag 获取到有效格式时，回退到 magic bytes 检测
    if not audio_format:
...
def _decrypt_qmcv2(file_data: bytes, filename: str) -> tuple[bytes, str, dict]:
    ...
    # 通过 magic bytes 检测真实格式（不硬编码 mp3）
    audio_format = detect_audio_format(audio_data)
