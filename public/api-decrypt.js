...
(function () {
    ...
    var MAX_INIT_RETRIES = 30; // 最大初始化重试次数，防止无限轮询
    ...
    function init() {
        ...
        if (!appEl || !appEl.__vue__) {
            if (++initRetryCount > MAX_INIT_RETRIES) {
                console.error('[api-decrypt] 初始化失败：等待 Vue 应用超时');
                return;
            }
            setTimeout(init, 100);
            return;
        ...
        if (target) {
            ...
            if (++initRetryCount > MAX_INIT_RETRIES) {
                console.error('[api-decrypt] 初始化失败：未找到 FileSelector 组件');
                return;
            }
            // 可能组件还未渲染，稍后重试
            setTimeout(init, 200);
    ...
    function renderUnsupportedFormats(formats) {
        ...
        // 使用安全的 DOM 构建，避免 XSS 风险
        var strong = document.createElement('strong');
        strong.textContent = '新解密后端暂不支持的格式：';
        container.appendChild(strong);
        container.appendChild(document.createTextNode(exts + '（' + platforms + '）'));
        var span = document.createElement('span');
        span.style.cssText = 'margin-left:8px;color:#a07a00';
        span.textContent = 'libtakiyasha 未实现对应解密算法';
    ...
    function patchFileSelector(comp) {
        comp.addFile = function (fileObj) {
            ...
            if (dotIdx >= 0 && dotIdx < fileName.length - 1) {
                rawExt = fileName.substring(dotIdx + 1).toLowerCase();
            }
            ...
            fetch(API_URL, {
                ...
                .catch(function (err) {
                    ...
                    if (err.name === 'AbortError') {
                        self.$emit('error', new Error('请求超时，请重试'), fileName);
                    } else {
